#include <iostream>
#include "HashNode.h"

template <class T1, class T2>
HashNode<T1, T2>::HashNode()
{
}

template <class T1, class T2>
HashNode<T1, T2>::HashNode(T1 k, T2 v)
{

  key = k;
  value = v;
}

template <class T1, class T2>
bool HashNode<T1, T2>::operator==(const HashNode<T1, T2> &oldnode)
{

  return (key == oldnode.key) ? true : false;
}

template <class T1, class T2>
bool HashNode<T1, T2>::operator!=(const HashNode<T1, T2> &oldnode)
{
  return (key == oldnode.key) ? false : true;
}

template <class T1, class T2>
T1 HashNode<T1, T2>::getKey()
{
  return key;
}

template <class T1, class T2>
void HashNode<T1, T2>::setValue(T2 v)
{
}

template <class T1, class T2>
T2 HashNode<T1, T2>::getValue()
{

  return value;
}

template <class T1, class T2>
std::ostream &operator<<(std::ostream &output, HashNode<T1, T2> &hn)
{
  output << hn.getKey() << ":" << hn.getValue();
  return output;
}
